...
describe('preflight for the new business-invariant indexes', () => {
  ...
  test('a plate held by two accounts and a plate duplicated inside one account are reported separately', async () => {
    ...
    // Cố ý seed thẳng mảng nhúng cũ: category này là PREFLIGHT cho DB chưa chạy
    // migrate:vehicles, nên nó phải soi `User.licensePlates` chứ không phải Vehicle.
    const first = await f.createUser();
  ...
  test('an unmapped vehicle class is reported but never blocks index creation', async () => {
    const building = await f.createBuilding();
    await f.createVehicleType(building._id, { code: 'UNMAPPED', category: null });
    await VehicleType.collection.insertOne({
      building: building._id, code: 'BOGUS', name: 'Bogus', category: 'spaceship', isActive: true,
    });
