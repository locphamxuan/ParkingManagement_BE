 ...
 */
const { normalizeVehicleClass } = require('../constants/vehicleClass');

const idOf = (value) => (value?._id ? value._id : value) || null;

/** Snapshot điều khoản từ package tại thời điểm trả tiền. */
const buildPurchasedTerms = (pkg, { vehicleClass = null, purchasedAt = new Date(), backfilled = false } = {}) => ({
  ...
  vehicleClass: normalizeVehicleClass(vehicleClass ?? pkg?.vehicleType?.vehicleClass),
  purchasedAt,
  backfilled,
});
...
const effectiveTerms = (subscription) => {
  ...
  if (hasStored) {
    return {
      ...
      vehicleClass: stored.vehicleClass || null,
    };
  }
  ...
  return {
    ...
    vehicleType: idOf(pkg?.vehicleType),
    vehicleClass: normalizeVehicleClass(pkg?.vehicleType?.vehicleClass),
  };
};
