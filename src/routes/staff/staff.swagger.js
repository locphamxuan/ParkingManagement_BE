 ...
 *   get:
 *     tags: [Staff - Feedback]
 *     summary: List feedback for the current scope
 *     description: Returns feedback for managers by building scope. Staff accounts are not allowed to use this endpoint in the current service logic.
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: query
 ...
 *       403: { content: { application/json: { schema: { $ref: '#/components/schemas/ErrorResponse' } } } }
 * /api/staff/feedbacks/{id}/resolve:
 *   patch:
 *     tags: [Staff - Feedback]
 *     summary: Resolve feedback
 *     description: Only manager-role accounts can resolve feedback in the current implementation.
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
 ...
 *             type: object
 *             properties:
 *               staffReply: { type: string, maxLength: 1000, example: Thank you for your feedback. We have resolved the issue. }
 ...
 *       - in: query
 *         name: status
 *         schema: { type: string, enum: [open, investigating, escalated, resolved, closed] }
 *       - in: query
 *         name: severity
 *         schema: { type: string, enum: [medium, high, critical] }
 *       - in: query
 *         name: page
 *         schema: { type: integer, minimum: 1, default: 1 }
 *       - in: query
 ...
 *                               code: { type: string, example: INC-20260612-001 }
 *                               type: { type: string, example: Lost ticket }
 *                               target: { type: string, example: 59G2-038.80 }
 *                               note: { type: string, example: Driver lost parking ticket during exit. }
 *                               building: { $ref: '#/components/schemas/ObjectId' }
 *                               severity: { type: string, enum: [medium, high, critical], example: high }
 *                               status: { type: string, enum: [open, investigating, escalated, resolved, closed], example: open }
 ...
 *             required: [type]
 *             properties:
 *               type: { type: string, maxLength: 150, example: Lost ticket }
 *               incidentType: { type: string, maxLength: 150, example: Lost ticket }
 *               target: { type: string, maxLength: 200, example: 59G2-038.80 }
 *               note: { type: string, maxLength: 1000, example: Driver lost parking ticket during exit. }
 *               buildingId: { type: string, format: objectId }
 *               severity: { type: string, enum: [medium, high, critical], example: high }
 *               parkingSessionId: { type: string, format: objectId }
 *               penaltyFee: { type: number, minimum: 0, example: 50000 }
 *               paymentMethod: { type: string, enum: [cash, wallet, qr], example: cash }
 *     responses:
 *       201:
